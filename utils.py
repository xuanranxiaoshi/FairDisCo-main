...
def getOneHot(df):
    """
    将传入的数据矩阵根据每一列的取值进行独热处理，得到新的特征表示
    """
    C = df.values
    # df.nunique() 每一列不同取值的个数
    D = df.nunique().values.tolist()
    X = []
    for col in df.columns:
        X.append(pd.get_dummies(df[col]).values)
    # 将样本在每一列的取值转化为独热码，最后将每一列的独热码拼接起来形成最后的特征向量
    X = np.concatenate(X, axis=1)
    # 转换后的样本特征； 实际数据； 每一列的取值个数
    return X, C, D


def getBinary(df, cols):
    """
    将df中指定的列的数据二进制化，大于中位数的为1， 小于中位数的为0
    """
    labels = df[cols].apply(lambda s: np.median(s)).values
    x = df[cols].values
    xs = np.zeros_like(x)
    for j in range(len(labels)):
        # 如果该列的最大值等于其中位数，则数据维持不变
        if x[:, j].max() == labels[j]:
            xs[:, j] = x[:, j]
        else:
...
def getDataset(df, S, Y, num_train):
    ...
    # LabelEncoder().fit_transform(col): 对数据集进行编码
...
def loadHealth():
    ...
    drop_cols = ['age_%d5' % (i) for i in range(0, 9)] + ['sexMALE', 'sexFEMALE', 'sexMISS', 'age_MISS', 'trainset',
                                                          'DaysInHospital', 'MemberID_t', 'YEAR_t']
...
def loadMnist(color=True):
    transform = transforms.Compose(
        [transforms.ToPILImage(), transforms.Scale(64), transforms.CenterCrop(64), transforms.ToTensor()])
...
all_datasets = ['Adult-sex', 'Adult-race', 'Health', 'Bank', 'German', 'Creadit', 'Student', 'Compas-sex',
                'Compas-race']
